...
export class ChangeDetectionTrackingZoneSpec implements ZoneSpec {
  ...
  static get() { return Zone.current.get('ChangeDetectionTrackingZone'); }

  private getTasksFor(type: string): { tasks: Task[], tasks$: Subject<TaskChange> } {
    switch (type) {
      case 'microTask':
  ...
  startTracking(stable$: Observable<boolean>) {
    ...
    this.performance$ = tasksInvoked$.pipe(map(
      (t: any) => ({
        task: t.task,
